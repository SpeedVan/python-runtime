...
def app_init_funcs(app: Flask):
    ...
    app.config['func_cache'] = load_funcs(
        app.config['orginal_sys_path'], app.config['func_json'])
...
def load(orginal_sys_path, sys_path, entrypoint):
    ...
    if type(func_sign.return_annotation) == tuple:
        params_struct = dict(
            map(lambda kv: (kv[0], str(kv[1].annotation)), func_sign.parameters.items()))
        result_struct = dict(map(lambda v: (v.__name__, str(
            v.__supertype__)), func_sign.return_annotation))
        func_proxy = lambda **kvargs: dict(
            zip(result_struct.keys(), func(**kvargs)))
